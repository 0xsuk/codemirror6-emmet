...
export const balanceOutward: StateCommand = ({ state, dispatch }) => {
    ...
    for (const sel of state.selection.ranges) {
        ...
        if (ranges) {
            ...
            const targetRange =
                ranges.find(
                    (r) => rangeContains(r, sel) && !rangesEqual(r, sel)
                ) || sel;
            nextSel.push(
                EditorSelection.range(targetRange.from, targetRange.to)
            );
        } else {
            nextSel.push(sel);
        }
    ...
    for (const sel of state.selection.ranges) {
        ...
        if (ranges) {
            ...
            if (ix < ranges.length - 1) {
                ...
                targetRange =
                    ranges.slice(ix).find((r) => rangeContains(r, sel)) || sel;
            }
...
function getOutwardRanges(
    state: EditorState,
    pos: number
): RangeObject[] | undefined {
